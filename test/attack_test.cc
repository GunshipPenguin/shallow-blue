...
TEST_CASE("Board::getWhiteAttacks and Board::getBlackAttacks work properly") {
  ...
  SECTION("Attack generation works for a position with multiple pieces having overlapping attacks") {
    ...
    REQUIRE(board.getWhiteAttacks() == U64(0x825d027afa173d1b));
    REQUIRE(board.getBlackAttacks() == U64(0x5aeff0bdff7b4c04));
  }

  SECTION("Attack generation takes en passant into account") {
    board.setToFen("8/8/8/8/3p4/8/8/8 w - d3");
    REQUIRE(board.getWhiteAttacks() == U64(0x80000));

    board.setToFen("8/8/8/8/3P4/8/8/8 w - d3");
    REQUIRE(board.getBlackAttacks() == U64(0x80000));
  }
}
