...
void Book::_initMoves(std::string path) {
  ...
  while (bookFile) {
    ...
    auto moveVec = std::vector<std::pair<unsigned short, unsigned short>>(1, moveWeightPair);
...
Move Book::getMove(const Board& board) const {
  ...
  if (lookup != _content.end()) {
    ...
    throw new std::logic_error("Error in calculating weighted average");
  } else {
    throw new std::logic_error("Position is not in book");
  }
}
